...
def test_overall_flow(user, user_password):
    ...
    custom_sessions = list(UserSession.objects.filter(user=user).order_by("pk"))
    assert custom_sessions[0].user_agent == "Mozilla Firefox"
    assert custom_sessions[1].user_agent == "Nyxt"
    for client in [firefox, nyxt]:
        resp = client.get(reverse("usercustom_sessions_list"))
        assert resp.status_code == 200
    resp = firefox.post(reverse("usercustom_sessions_list"))
    assert resp.status_code == 302
    assert UserSession.objects.filter(user=user).count() == 1
    assert UserSession.objects.filter(user=user, pk=custom_sessions[0].pk).exists()
    assert not UserSession.objects.filter(user=user, pk=custom_sessions[1].pk).exists()
    resp = nyxt.get(reverse("usercustom_sessions_list"))
    assert resp.status_code == 302
    assert resp["location"] == reverse("account_login") + "?next=" + reverse(
        "usercustom_sessions_list"
    )
