...
from allauth.usercustom_sessions.middleware import Usercustom_sessionsMiddleware
from allauth.usercustom_sessions.models import UserSession
from allauth.usercustom_sessions.signals import session_client_changed


def test_mw_without_request_user(rf, db, settings):
    settings.USERcustom_sessions_TRACK_ACTIVITY = True
    mw = Usercustom_sessionsMiddleware(lambda request: None)
    request = rf.get("/")
    mw(request)
...
def test_mw_with_request_user(rf, db, settings, user, track_activity):
    settings.USERcustom_sessions_TRACK_ACTIVITY = track_activity
    mw = Usercustom_sessionsMiddleware(lambda request: None)
    request = rf.get("/")
    request.user = user
...
@override_settings(USERcustom_sessions_TRACK_ACTIVITY=True)
