...
if not allauth_settings.USERcustom_sessions_ENABLED:
    raise ImproperlyConfigured(
        "allauth.usercustom_sessions not installed, yet its models are imported."
    )
...
class UserSessionManager(models.Manager):
    def purge_and_list(self, user):
        ret = []
        custom_sessions = UserSession.objects.filter(user=user)
        for session in custom_sessions.iterator():
...
class UserSession(models.Model):
    ...
    def _session_store(self, *args):
        ...
        return engine.custom_sessionstore(*args)
    ...
    def end(self):
        engine = import_module(settings.SESSION_ENGINE)
        store = engine.custom_sessionstore()
