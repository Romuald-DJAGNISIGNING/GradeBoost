...
def test_flow(client, user, user_password, headless_reverse, settings):
    ...
    resp = client.get(headless_reverse("headless:usercustom_sessions:custom_sessions"))
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = client.delete(
        headless_reverse("headless:usercustom_sessions:custom_sessions"),
        data={"custom_sessions": [session_pk]},
        content_type="application/json",
    )
