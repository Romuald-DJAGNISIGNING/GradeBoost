...
def build_urlpatterns(client):
    ...
    if allauth_settings.USERcustom_sessions_ENABLED:
        from allauth.headless.usercustom_sessions import urls as usercustom_sessions_urls

        patterns.append(
            path(
                "",
                include(
                    (usercustom_sessions_urls.build_urlpatterns(client), "headless"),
                    namespace="usercustom_sessions",
                ),
            )
        )
