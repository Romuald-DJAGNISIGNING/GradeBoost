...
- Session management:
  - Listing all custom_sessions for a user.
  - Signing out of any of those custom_sessions.


# Browser Usage

For web applications running in a browser, routing needs to be setup correctly
such that the custom_sessions initiated at the backend are accessible in the frontend.

## Routing

When using the API in a browser context, regular Django custom_sessions are used, along
...
For app based usage, cookies play no role, yet, custom_sessions are still used. When a
...
the backend. Unauthenticated custom_sessions are also needed to remember state while
